...
def _minimal_bundle(lab: bool = False) -> LessonPlanBundle:
    """Build a minimal LessonPlanBundle for Word export tests."""
    if lab:
...
class ExportWordTests(unittest.TestCase):
    def test_export_creates_docx_file(self) -> None:
        bundle = _minimal_bundle(lab=False)
        with tempfile.TemporaryDirectory() as tmp_dir:
            result_path = export_bundle_to_word(bundle, tmp_dir)

            self.assertTrue(result_path.exists())
            self.assertTrue(str(result_path).endswith(".docx"))
            self.assertGreater(result_path.stat().st_size, 0)

    def test_export_word_with_lab_bundle(self) -> None:
